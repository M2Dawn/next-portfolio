...
export default function Contact() {
  ...
  const copyEmail = async () => {
    try {
      ...
      setCopied(true);
      sendGAEvent('event', 'copy_email_click', { method: 'button' });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess(true);
      sendGAEvent('event', 'form_submit_success', { form_name: 'contact_form' });
      form.reset();
  ...
  return (
    <section className="relative py-32 bg-[#050507] overflow-hidden border-t border-white/5" id="contact">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none flex items-center justify-center">
        <div className="w-[800px] h-[800px] rounded-full bg-[radial-gradient(circle,rgba(34,72,163,0.04)_0%,transparent_50%)] blur-[100px]"></div>
      </div>
        ...
        <Reveal>
          <div className="flex flex-col items-center text-center w-full mb-12">
            ...
            <div className="flex items-center gap-4 mt-6 sm:mt-0">
              <a href="https://www.linkedin.com/in/HossamSabryDev" target="_blank" rel="noopener noreferrer" onClick={() => sendGAEvent('event', 'social_click', { network: 'linkedin' })} className="w-12 h-12 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center hover:bg-[#0A66C2]/10 hover:border-[#0A66C2] transition-all duration-300">
              ...
              <a href="https://github.com/M2Dawn" target="_blank" rel="noopener noreferrer" onClick={() => sendGAEvent('event', 'social_click', { network: 'github' })} className="w-12 h-12 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all duration-300">
            ...
            <div className="rounded-[calc(2.5rem-3px)] bg-[var(--color-bg-card)] shadow-[inset_0_1px_1px_rgba(255,255,255,0.08)] p-8 md:p-12 border border-white/5 relative overflow-hidden text-left">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-6 relative z-10">
                ...
                <button 
                  ...
                  className={`relative overflow-hidden w-full py-4 rounded-xl font-bold text-white transition-all duration-300 mt-4 ${success ? 'bg-green-500 shadow-[0_0_20px_rgba(74,222,128,0.2)]' : errorStatus ? 'bg-red-500' : 'bg-brand hover:bg-brand-light shadow-[0_0_30px_rgba(34,72,163,0.2)] hover:shadow-[0_0_40px_rgba(34,72,163,0.4)] hover:-translate-y-0.5'}`}
                >
